...
struct NodeTrie<T> {
    key: Box<Vec<u8>>,
    children: Option<Box<BTreeMap<u8, NodeTrie<T>>>>,
    cnt: u8,
    value: Option<T>,
}
impl<T> RadixTrie<T> {
    ...
    pub fn insert(&mut self, key: &[u8], value: T) {
        ...
        match self.root.get_mut(&key[0]) {
            ...
            None => {
                self.root.insert(
                    key[0],
                    NodeTrie::new(key, Some(value), Some(Box::new(BTreeMap::new())), 0),
                );
            }
        }
    }
...
impl<T> NodeTrie<T> {
    fn new(key: &[u8], value: Option<T>, children: Option<Box<BTreeMap<u8, NodeTrie<T>>>>, cnt: u8) -> Self {
        NodeTrie {
            key: Box::new(Vec::from(key)),
            value,
            children,
            cnt,
        }
    ...
    fn insert(&mut self, key: &[u8], value: Option<T>) {
        ...
        if idx < key.len() {
            if idx < self.key.len() {
                ...
                self.cnt = 1;
                self.children = Some(Box::new(BTreeMap::new()));
                unsafe {
            ...
            if let Some(next) = unsafe { self.children.as_mut().unwrap_unchecked().get_mut(&key[idx]) } {
                ...
                unsafe {
                    self.children.as_mut().unwrap_unchecked().insert(
                        key[idx],
                        NodeTrie::new(&key[idx..], value, Some(Box::new(BTreeMap::new())), 0),
                    );
                }
            }
